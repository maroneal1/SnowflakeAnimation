...
public:
    ...
    * \returns Pointer to animation channel */
	...
	/// \brief Setter for our Text Box \param textBox textBox we are setting to
	void SetTextBox(std::shared_ptr<CTextBubbleAdapter> textBox) { mTextBox = textBox; }
	/// \brief Getter for our text box \returns shared pointer to text bubble adapter
	std::shared_ptr<CTextBubbleAdapter> GetTextBox() { return mTextBox; }
...
private:
	...
	/// Pointer to text box encapsulator
	std::shared_ptr<CTextBubbleAdapter> mTextBox;
};
