...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_COMMAND(ID_EDIT_ADDTEXT, &CMainFrame::OnEditEditHarold)
	ON_COMMAND(ID_EDIT_EDITSPARTY32786, &CMainFrame::OnEditEditSparty)
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
void CMainFrame::ChangeText(std::wstring str)
{
	for (auto actor : *mPicture)
	{
		if (actor->GetName() == str)
		{
			CTextBubbleDlg dlg;
			auto textBox = actor->GetTextBox();
			textBox->SetDialogTextBox(dlg);
			if (dlg.DoModal() == IDOK)
			{
				dlg.Take();
				mPicture->UpdateObservers();
...
void CMainFrame::OnEditEditHarold()
{
	ChangeText(L"Harold");
}
...
void CMainFrame::OnEditEditSparty()
{
	ChangeText(L"Sparty");
}
