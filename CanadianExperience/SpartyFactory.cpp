...
std::shared_ptr<CActor> CSpartyFactory::Create()
{
	...
	auto sword = make_shared<CImageDrawable>(L"Sword", L"images/sword.png");
	sword->SetCenter(Point(12, 218));
	sword->SetPosition(Point(70, 90));
	sword->SetRotation(-.5);
	larm->AddChild(sword);
	...
	auto textBox = make_shared<CTextBubbleAdapter>(L"Text box");

	textBox->SetPosition(Gdiplus::Point(75, 0));
	headt->AddChild(textBox);
	actor->SetTextBox(textBox);

	actor->AddDrawable(sword);
